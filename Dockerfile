...
RUN ln -s /usr/share/zoneinfo/UTC /etc/localtime \
    ...
    && echo '[nuxsfm]' >> /etc/pacman.conf \
    && echo 'SigLevel = Optional TrustAll' >> /etc/pacman.conf \
    && echo 'Server = http://stephane-lb.github.io/NuxSFM/$arch' >> /etc/pacman.conf \ 
    && pacman -Syu \ 
    && pacman -S openMVG --noconfirm --force
#    ddopenMVG pmvs bundler visualsfm
